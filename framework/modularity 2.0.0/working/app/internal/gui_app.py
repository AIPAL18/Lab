...
from framework.globals.IApplication import BaseApplication, IApplication
...
from framework.ui.IUiEngine import IUiEngine
...
class GuiApp(BaseApplication):   # enable_shared_from_this ???
    ...
    def perform(self):
        ...
        engine = ioc().resolve("app", "", IUiEngine).qmlAppEngine()
    ...
    def finish(self):
        ...
        if globalThreadPool:
            print("activeThreadCount", globalThreadPool.activeThreadCount())
            globalThreadPool.waitForDone()
        
        ioc().resolve("app", "", IUiEngine).quit()
