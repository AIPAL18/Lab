...
class GuiApp(BaseApplication):   # enable_shared_from_this ???
    ...
    def perform(self):
        ...
        engine = ioc().resolve(IUiEngine).qmlAppEngine()

        main_qml_file = "Main.qml"
        url = QUrl("qrc:/qml/" + main_qml_file)

        # I don't know what it's about... (it's connecting the sceneGraphError handler (after the creation of the window))
