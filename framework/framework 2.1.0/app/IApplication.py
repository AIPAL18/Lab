...
from framework.globals import modularity
from framework.globals.types import Version
...
@modularity.INTERFACE_ID
class IApplication(modularity.ModuleExportInterface):
    ...
    @abstractmethod
    def ioc(self: IApplication) -> modularity.ModulesIoC: ...
...
class BaseApplication(IApplication):
    ...
    def appBuild(self: BaseApplication) -> str:
        return str()

    def appRevision(self: BaseApplication) -> str:
        return str()
    ...
    def restart(self: BaseApplication) -> None:
        program = QCoreApplication.instance().arguments()[0]

        # NOTE: remove the first argument - the program name
        arguments = QCoreApplication.instance().arguments()[1:]
    ...
    @override
    def ioc(self: BaseApplication) -> modularity.ModulesIoC:
        return modularity.ioc()
    ...
    def notify(self: BaseApplication, object: QObject, event: QEvent) -> bool:
        return QGuiApplication.instance().notify(object, event)
